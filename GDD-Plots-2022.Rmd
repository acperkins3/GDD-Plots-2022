...
for (row in 1:nrow(New)) {
  if (row == 1) {
    New[row, "K25-K32"] <- New[1, "GDDDaily"]
  }
  else {
    New[row, "K25-K32"] <- New[(row-1), "K25-K32"] + New[row, "GDDDaily"]
  }
}
...
HANExisting <- read.csv("Hancock_existing.csv") %>%
  select(Date, `K25-K32`) %>%
  mutate(Date = as.Date(Date, "%m/%d/%y"))

Merged <- merge(WMARSExisting, HANExisting, by = "Date", all=TRUE) %>%
  gather(M800, `K25-K32`, key = "Field", value = "GDDs") %>%
  mutate(Field = case_when(
    Field == "M800" ~ "West Madison (M800)",
    Field == "K25-K32" ~ "Hancock (K25 - K32)"
  ))

Merged$Field <- factor(Merged$Field, levels = c("West Madison (M800)", "Hancock (K25 - K32)"))
...
```{r}
WMARSExisting <- read.csv("WMARS_existing.csv") %>%
  select(Date, H102, B504) %>%
  mutate(Date = as.Date(Date, "%m/%d/%y")) %>%
  gather(H102, B504, key = "Field", value = "GDDs")
