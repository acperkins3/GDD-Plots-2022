...
data <- read_html(paste("https://legacy.enviroweather.msu.edu/run.php?stn=hck&mod=w_sum&da1=28&mo1=4&da2=", (as.numeric(substr(Date, 9, 10))-1), "&mo2=", substr(Date, 6, 7), "&yr=2021&mc=604&ds=cd", sep="")) %>% 
  html_elements("td") %>%
  html_text2()
...
data <- read_html(paste("https://legacy.enviroweather.msu.edu/run.php?stn=alt&mod=w_sum&da1=10&mo1=5&da2=", (as.numeric(substr(Date, 9, 10))-1), "&mo2=", substr(Date, 6, 7), "&yr=2021&mc=604&ds=cd", sep="")) %>% 
  html_elements("td") %>%
  html_text2()
...
for (row in 3:nrow(New)) {
  if (row == 3) {
    New[row, "H200"] <- New[3, "GDDDaily"]
  }
  else {
    New[row, "H200"] <- New[(row-1), "H200"] + New[row, "GDDDaily"]
  }
}
...
Merged <- WMARSExisting %>%
  gather(M800, key = "Field", value = "GDDs") %>%
  mutate(Field = case_when(
    Field == "M800" ~ "West Madison (M800)",
  ))

Merged$Field <- factor(Merged$Field, levels = c("West Madison (M800)"))
